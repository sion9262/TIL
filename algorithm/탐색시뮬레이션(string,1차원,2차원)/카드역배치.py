...
"""
# 카드 역배치
def card_change(numbers, i, j):
    # 해당 범위 추출
    change = numbers[i:j+1]
    # 리스트 역변환
    change = change[::-1]
    # 대입
    numbers[i:j+1] = change
    return numbers

def solve(numbers, changes):
    for i, j in changes:
        numbers = card_change(numbers, i, j)
    numbers = list(map(str, numbers))
    return ' '.join(numbers[1:])


numbers = [i for i in range(0, 21)]
changes = [list(map(int, input().split())) for _ in range(10)]

print(solve(numbers, changes))
